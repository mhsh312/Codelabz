...
export const createTutorial =
  tutorialData => async (firebase, firestore, dispatch, history) => {
    try {
      ...
      const { title, summary, owner, created_by, is_org, tutorialImage } =
        tutorialData;
      const setData = async () => {
        ...
        const fileType = tutorialImage?.substring(
          tutorialImage.indexOf(":") + 1,
          tutorialImage.indexOf("/")
        );
        ...
        await document.set({
          ...
          featured_image: fileType === "image" ? tutorialImage : "",
          icon: "",
          url: "",
...
export const likeTutorial =
  (tutorial_id, likedBy) => async (firestore, dispatch) => {
    try {
      dispatch({ type: actions.LIKE_TUTORIAL_START });
      const docref = await firestore
        .collection("tutorials")
        .doc(tutorial_id)
        .get();
      const tutorial = await docref.data();
      if (tutorial?.likers.includes(likedBy)) {
        tutorial.likers = tutorial?.likers.filter(like => like !== likedBy);
      } else {
        tutorial.likers = [...tutorial.likers, likedBy];
      ...
      await firestore
        ...
        .then(() => {
          dispatch({ type: actions.LIKE_TUTORIAL_SUCCESS });
        });
    } catch (e) {
      console.log("LIKE_TUTORIAL_FAIL", e);
      dispatch({ type: actions.LIKE_TUTORIAL_FAIL, payload: e.message });
    }
  };

export const dislikeTutorial =
  (tutorial_id, dislikedBy) => async (firestore, dispatch) => {
    try {
      dispatch({ type: actions.DISLIKE_TUTORIAL_START });
      const docref = await firestore
        .collection("tutorials")
        .doc(tutorial_id)
        .get();
      const tutorial = await docref.data();
      if (tutorial?.dislikers.includes(dislikedBy)) {
        tutorial.dislikers = tutorial?.dislikers.filter(
          dislike => dislike !== dislikedBy
        );
      } else {
        tutorial.dislikers = [...tutorial.dislikers, dislikedBy];
        tutorial.likers = tutorial?.likers.filter(like => like !== dislikedBy);
      }
      await firestore
        .collection("tutorials")
        .doc(tutorial_id)
        ...
        .then(() => {
          dispatch({ type: actions.DISLIKE_TUTORIAL_SUCCESS });
        });
    } catch (e) {
      console.log("LIKE_TUTORIAL_FAIL", e);
      dispatch({ type: actions.DISLIKE_TUTORIAL_FAIL, payload: e.message });
    }
  };
